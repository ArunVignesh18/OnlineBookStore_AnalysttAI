...
For test purposes, I have created an admin account and 2 user accounts.
...
The books are displayed in the Dashboard/Home Page with search and filter options for details of the books. To view a particular book's details, there is button called 'View Book' button below each book.

ADMIN

    1. The admin can add new books to the application with its details such as - title, author, description, Price, Rating, Genres, Publication Date. This can be done in 'Add Book' page.

    2. The admin can add new genres to the genres list which is not available before, and this can be done in the 'Add Genre' page.

    3. The admin can remove any existing genre from the genres list by clicking the 'Remove Book' button. This can also be done in the 'Add Genre' page.

    4. The admin can remove any existing book, and doing this will remove the book permanently. i.e. It remove the book from the cart too, if someone added to the cart before. This can be done by clicking the 'Remove Book' button in the dashboard page.
...
There is a 'Buy Now' button in cart page at the bottom, which don't have any functionality. Since we can't buy books in this project, I didn't place any function for that button.
